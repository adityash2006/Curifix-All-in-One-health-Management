...
export default function AiChat() {
    ...
    return (<>
                   ...
                   {newChat && <div>
                    <p className="font-bold text-4xl z-10 relative top-35 instrument tracking-wide text-yellow-600 text-center">Start a new Chat !</p></div>}
         ...
         
            {
                prevChats?.slice(1).slice(0,-1).map((chat,idx)=>
                    <div className={chat.role==="user"?"userprompt":"aiprompt"} key={idx}>
                        {
                            chat.role==="user"?
                            <span className="inline-block text-lg text-black text-left max-w-110 rounded-lg mt-2 p-2 px-3 bg-[#f7ffb1] ">{chat.content} </span>:
